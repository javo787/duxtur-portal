...
- [Тёмная тема (Dark Mode)](#тёмная-тема-dark-mode)
...
На платформе реализована профессиональная тёмная тема, основанная на глубоких тёмно-синих оттенках (Deep Navy), что снижает нагрузку на глаза и придает интерфейсу премиальный вид.

- **Палитра:** Использование цветового пространства OKLCH для точной настройки контрастности.
- **Философия:** Отказ от чистого черного в пользу темно-синего (#0f172a / oklch) для улучшения читаемости.
- **Технологии:** CSS Variables, Tailwind CSS 4, автоматическое определение системной темы.

Подробности реализации и планы развития доступны в файле [DARK_MODE.md](./DARK_MODE.md).
...
Проект Duxtur.org спроектирован с учетом лучших практик медицинского SEO (E-E-A-T):

- **Hreflang & i18n:** Полная поддержка мультиязычности с правильной индексацией региональных версий.
- **Structured Data:** Использование JSON-LD для `MedicalClinic`, `Physician`, `Article`, `FAQPage` и `BreadcrumbList`.
- **Dynamic Sitemaps:** Автоматическая генерация карт сайта для всех типов контента.
- **Performance:** Оптимизация шрифтов, изображений и использование ISR для быстрой загрузки.

Подробную информацию о реализованных стратегиях и планах развития читайте в [SEO.md](./SEO.md).

---

## ⚡ Оптимизация производительности

Для обеспечения максимальной скорости работы в регионах с медленным интернетом (Центральная Азия) реализован комплекс мер:

- **Изображения:** Приоритетная загрузка (LCP), адаптивные размеры, CDN-оптимизация через Cloudinary.
- **База данных:** Оптимизированные индексы MongoDB, использование `.lean()` для всех запросов на чтение.
- **Кэширование:** ISR (Incremental Static Regeneration) и SWR на уровне Edge Network.
- **Безопасность:** Настроенные CSP и современные HTTP-заголовки.

Полный список оптимизаций и планов доступен в [OPTIMIZATION.md](./OPTIMIZATION.md).
