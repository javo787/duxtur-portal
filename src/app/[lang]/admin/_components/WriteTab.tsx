...
const MODES: { id: Mode; icon: string; title: string; desc: string; hint: string; limit: number; minLimit: number }[] = [
  { id: 'write',   icon: '✍️', title: 'Написать',   desc: 'Опишите тему своими словами — AI структурирует статью',     hint: 'Напишите черновик: о чём статья, симптомы, что важно знать пациентам. Не нужно быть идеальным — AI всё оформит.', limit: 3000,  minLimit: 100 },
  { id: 'process', icon: '📄', title: 'Обработать', desc: 'Вставьте научную статью — AI адаптирует для пациентов',      hint: 'Вставьте текст из PubMed, ВОЗ, учебника или любого медицинского источника. AI упростит язык и структурирует для читателей.', limit: 12000, minLimit: 200 },
  { id: 'translate',icon: '🌐',title: 'Перевести',  desc: 'Вставьте готовый текст — AI переведёт профессионально',     hint: 'Вставьте медицинскую статью на любом языке. AI переведёт на выбранный язык с сохранением медицинской точности.', limit: 12000, minLimit: 100 },
];
...
export function WriteTab({ lang }: { lang: string }) {
  ...
  const handleProcess = async () => {
    ...
    setIsLoading(true);
    const fn = mode === 'write' ? processMedicalDraft : mode === 'process' ? processMedicalArticle : translateMedicalArticle;
  ...
  const handlePublish = async () => {
    ...
    setIsSaving(false);
    if (result.success) { setPublishedSlug(result.slug || ''); setStep(1); setDraft(''); setArticle(null); }
      ...
      {step === 1 && (
        <>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
              {MODES.map((m) => (
                <button key={m.id} onClick={() => { setMode(m.id); setDraft(''); }}
                  className={`p-4 rounded-2xl border-2 text-left transition ${mode === m.id ? 'border-blue-500 bg-blue-50' : 'border-gray-100 hover:border-blue-200 bg-gray-50'}`}>
                  <span className="text-2xl block mb-2">{m.icon}</span>
                  <p className={`font-extrabold text-sm mb-1 ${mode === m.id ? 'text-blue-700' : 'text-gray-900'}`}>{m.title}</p>
          ...
          <div className="bg-white rounded-3xl border border-gray-100 shadow-sm p-6">
            ...
            <textarea
              className={`w-full h-72 p-5 text-base border-2 rounded-2xl outline-none transition resize-none placeholder-gray-300 leading-relaxed ${isOverLimit ? 'border-red-400 bg-red-50' : 'border-gray-200 focus:border-blue-500'}`}
              placeholder={mode === 'write' ? 'Например: Мигрень — это интенсивная головная боль...' : mode === 'process' ? 'Вставьте текст научной статьи...' : 'Вставьте медицинский текст для перевода...'}
              value={draft}
            ...
            <div className="flex items-center justify-between mt-3 flex-wrap gap-3">
              <div className="flex items-center gap-3">
                <span className={`text-sm font-bold ${isOverLimit ? 'text-red-500' : isUnderMin ? 'text-gray-400' : 'text-green-600'}`}>
              ...
              <button onClick={handleProcess} disabled={isLoading || isUnderMin || isOverLimit}
                ...
                {isLoading ? (
                  <><svg className="animate-spin h-5 w-5" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"/><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z"/></svg> AI обрабатывает...</>
                ) : mode === 'write' ? '✨ Создать статью' : mode === 'process' ? '📄 Адаптировать' : '🌐 Перевести'}
              </button>
            </div>
      ...
      {step === 2 && article && (
        <div className="space-y-6 pb-20">
          ...
          <div className="bg-white rounded-3xl p-8 border border-gray-100 shadow-sm">
            ...
            <input value={article.title || ''} onChange={(e) => setArticle((a: any) => ({ ...a, title: e.target.value }))}
          ...
          <div onClick={() => fileInputRef.current?.click()}
            ...
            {article.image ? (
              ...
              <div className="text-center p-6">
                {isUploading ? <p className="font-bold text-blue-600">Загрузка...</p> : (
          ...
          <div className="grid grid-cols-1 gap-6">
            <Section title="📋 Обзор" content={article.overview} onChange={(v) => setArticle((a: any) => ({ ...a, overview: v }))} />
            {[1,2,3,4,5].map((i) => {
              if (!article[`section${i}_title`] && !article[`section${i}_content`]) return null;
              return (
                <Section key={i} title={article[`section${i}_title`] || `Раздел ${i}`} content={article[`section${i}_content`]}
