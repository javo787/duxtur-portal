...
export default function ClinicCard({ clinic, lang }: { clinic: any, lang: string }) {
  ...
  return (
    <motion.div
      ...
      className="h-full"
    >
      <Link href={`/${lang}/clinic/${clinic.slug}`} className="bg-white rounded-[2.5rem] overflow-hidden border border-slate-100 shadow-sm hover:shadow-2xl hover:shadow-blue-900/5 hover:-translate-y-2 transition-all duration-500 group flex flex-col h-full">
        {/* Cover */}
        <div className="relative aspect-video w-full overflow-hidden">
          ...
          {/* Logo Overlay */}
          <div className="absolute bottom-4 left-4 w-12 h-12 bg-white rounded-2xl p-0.5 shadow-xl group-hover:scale-110 transition-transform duration-500">
        ...
        <div className="p-5 sm:p-6 flex flex-col flex-1">
           ...
           <h3 className="text-base sm:text-lg font-black text-slate-900 group-hover:text-blue-600 transition-colors line-clamp-1 mb-2 font-display">
             {name}
           </h3>
           ...
           <div className="flex items-center gap-1 text-xs font-bold text-slate-400 mb-4">
              ...
              <span className="opacity-70">{clinic.doctorIds?.length || 0} {t('common.doctors')}</span>
           </div>

           <div className="flex flex-wrap gap-1.5 mt-auto">
              {clinic.specialties?.slice(0, 3).map((s: string) => (
                <span key={s} className="px-2.5 py-1 bg-slate-50 text-slate-500 rounded-lg text-[9px] font-bold uppercase tracking-tight group-hover:bg-blue-50 group-hover:text-blue-600 transition-colors">
                   {s}
                </span>
              ))}
