  ...
  return (
    <form ref={formRef} onSubmit={handleSubmit} className="bg-white/8 backdrop-blur-md border border-white/12 rounded-2xl p-3 flex flex-col md:flex-row gap-3">
      ...
      <div className="flex items-center gap-2 bg-white/8 border border-white/10 rounded-xl px-4 py-3 md:w-48 flex-shrink-0">
        ...
        <select name="city" defaultValue={searchParams.city} className="flex-1 bg-transparent text-sm font-medium text-white outline-none" aria-label={L('all_cities')}>
      ...
      <div className="flex-1 bg-white/8 border border-white/10 rounded-xl overflow-hidden">
        <SpecialtyAutocomplete
          ...
          onSelect={(value) => {
            // Программно меняем значение скрытого поля
            const input = formRef.current?.querySelector('input[name="specialty"]') as HTMLInputElement;
            if (input) input.value = value;
          }}
        />
        <input type="hidden" name="specialty" defaultValue={searchParams.specialty} />
      </div>
      ...
      <div className="flex items-center gap-2 bg-white/8 border border-white/10 rounded-xl px-4 py-3 md:w-52 flex-shrink-0">
        ...
        <select name="type" defaultValue={searchParams.type} className="flex-1 bg-transparent text-sm font-medium text-white outline-none" aria-label={L('consultation_type')}>
      ...
      {/* Сабмит */}
      <button type="submit" className="px-8 py-3 bg-blue-600 hover:bg-blue-700 text-white font-bold text-sm rounded-xl transition shadow-lg shadow-blue-900/30 flex-shrink-0 flex items-center gap-2">
